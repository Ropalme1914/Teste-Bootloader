...
/*
uint8_t sht4x_calcCrc8(uint8_t buf[], uint8_t from, uint8_t to)
{
...
uint8_t sht4x_singleTRhRead(uint8_t sht4x_address, uint8_t clockStretching, double *temperature, double *humidity)
{
	uint8_t status = 0;

	i2c_m_sync_set_slaveaddr(&I2C_0, sht4x_address, I2C_M_SEVEN);
	...
	if ( (buffer[2] == sht4x_calcCrc8(buffer, 0, 1)) && (buffer[5] == sht4x_calcCrc8(buffer, 3, 4)) )
	{
...
uint8_t sht4x_setPeriodicTRhSampling(uint8_t sht4x_address, uint8_t state)
{
	uint8_t status = 0;
...
uint8_t sht4x_periodicTRhRead(uint8_t sht4x_address, double *temperature, double *humidity)
{
	uint8_t status = 0;
...
uint8_t sht4x_singleDpRead(uint8_t sht4x_address, double *dewpoint)
{
	uint8_t status = 0;
	...
	if(buffer[2] == sht4x_calcCrc8(buffer, 0, 1))
	{
...
void sht4x_reset(uint8_t sht4x_address)
{
	i2c_m_sync_set_slaveaddr(&I2C_0, sht4x_address, I2C_M_SEVEN);
...
uint8_t sht4x_readIdentification(uint8_t sht4x_address, uint8_t identification[])
{
	uint8_t status = 0;
	...
	{
		if (buffer[8] == sht4x_calcCrc8(buffer, 0, 7))
		{
...
uint8_t sht4x_setHeater(uint8_t sht4x_address, uint8_t state)
{
	uint8_t status = 0;
...
uint8_t sht4x_setHeaterCurrent(uint8_t sht4x_address, uint8_t mA, uint8_t state)
{
	uint8_t status = 0;
	...
	command[2] = 0x08;
	command[3] = sendByte;
	command[4] = sht4x_calcCrc8(command, 2, 3);
...
uint8_t sht4x_setMeasurmentResolution(uint8_t sht4x_address, uint8_t temperatureBits, uint8_t rHumidityBits)
{
	uint8_t status = 0;
...
uint8_t sht4x_setMeasurmentPeriod(uint8_t sht4x_address, uint32_t millisec)
{
	uint8_t status = 0;
	...
	command[5] = sht4x_calcCrc8(command, 2, 4);
...
uint8_t sht4x_findSensors(uint8_t *sht4xsFound, uint8_t sht4xAddressList[])
{
	uint8_t status = 0;
	*sht4xsFound = 0;
	...
	for (uint8_t sht4x_address = 64; sht4x_address <= 71; sht4x_address++)
	{
		if ( (sht4x_readIdentification(sht4x_address, identification)) == 0 )
		{
			sht4xsFound++;
			sht4xAddressList[*sht4xsFound-1] = sht4x_address;
			//#ifdef CREATE_SHT4X_ID_LIST
			//sht4xIdList[sht4x_address-8] = identification[9];
			//#endif
		}
	}
	if (sht4xsFound == 0)
...
uint8_t sht4x_configure(uint8_t sht4x_address, uint8_t TResolution, uint8_t HResolution, uint32_t TRhPeriod)
{
	uint8_t status = 0;

	i2c_m_sync_set_slaveaddr(&I2C_0, sht4x_address, I2C_M_SEVEN);
	
	sht4x_reset(sht4x_address);
	delay_ms(1);
	...
	sht4x_setMeasurmentResolution(sht4x_address, TResolution, HResolution);
	delay_ms(1);
	
	if (TRhPeriod >= 1)
	{
		sht4x_setMeasurmentPeriod(sht4x_address, TRhPeriod);
		delay_ms(1);
		
		sht4x_setPeriodicTRhSampling(sht4x_address, 1);
	}
	else if (TRhPeriod == 0)
	{
		sht4x_setPeriodicTRhSampling(sht4x_address, 0);
	}
	else
	{
		status = 1;
...
uint8_t sht4x_single_blocking_TRh_read(uint8_t sht4x_address, double *temperature, double *humidity) 
{
	uint8_t status = 0;

	status = sht4x_order_measurement(sht4x_address);
	if (status)
	{
		return status;
	}
	delay_ms(10);
	return sht4x_read_measurement(sht4x_address, temperature, humidity);
}

uint8_t sht4x_order_measurement(uint8_t sht4x_address) 
{
	uint8_t status = 0;
	...
	uint8_t tx_buffer[1];	
	tx_buffer[0] = 0xFD;	// Measure T & RH with high precision (high repeatability)
	io_write(I2C_io, tx_buffer, 2);
	
	return status;
}

uint8_t sht4x_read_measurement(uint8_t sht4x_address, double *temperature, double *humidity) 
{
	uint8_t status = 0;
    ...
    if ( (rx_buffer[2] == sht4x_calcCrc8(rx_buffer, 0, 1)) && (rx_buffer[5] == sht4x_calcCrc8(rx_buffer, 3, 4)) )
    {
	    /**
